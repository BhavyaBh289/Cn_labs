...
char* generateMessageWithCRC(const char* data, const char* key) {
    char* message = (char*)malloc(strlen(data) + strlen(key) - 1); // Append zeros for division
    strcpy(message, data);
    strcat(message, "000"); // Append zeros for division
    ...
    strcpy(result, data);
    strcat(result, message + strlen(message) - keyLength + 1); // Append CRC remainder
    free(message);
    return result;
}
